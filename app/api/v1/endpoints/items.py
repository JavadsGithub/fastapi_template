...
async def create_item(payload: ItemCreate, service: ItemsService = Depends(get_items_service)):
    return await service.create_item(payload)

@router.get("/", response_model=List[ItemOut])
async def list_items(limit: int = 100, service: ItemsService = Depends(get_items_service)):
...
async def update_item(item_id: int, payload: ItemUpdate, service: ItemsService = Depends(get_items_service)):
